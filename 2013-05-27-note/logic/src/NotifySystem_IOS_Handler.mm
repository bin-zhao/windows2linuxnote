	...
	for (unsigned int i=0; i<number; i++)
	{
		...
		if (pNotifyInfo->mNotifyType == _TYPED_NOTIFY_DAILY_)
		{
			...
			{//判断系统是否支持本地通知
				...
				if (type == SysLangDataManager::_TYPED_SYSLANG_CN_)
					pNotification.alertBody = [NSString stringWithUTF8String:pNotifyInfo->mNotifyTextCN];//弹出的提示信息
				else
					pNotification.alertBody = [NSString stringWithUTF8String:pNotifyInfo->mNotifyTextEN];//弹出的提示信息
